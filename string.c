...
/*The  strpbrk() function locates the first occurrence in 
...
/*
int mystrcoll_l(const char *s, const char *s2, locale_t loc){
	int ret;
...
*/
/*The  mystrcoll() function compares the two strings s1 and s2.  It returns
...
* gram's current locale for category LC_COLLATE.
*/
/*int mystrcoll(const char *s, const char *s2){
	return mystrcoll_l(s, s2, __current_locale());
}
*/
/*
 ...
 */
/*size_t mystrxfrm(char *dst, const char *src, size_t n){
	...
	if (n != 0) {
		...
		(void)mymemcpy(dst, src, copysize);
		dst[copysize] = 0;
	}
	return (srclen);
}
